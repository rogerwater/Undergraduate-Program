...
if __name__ == "__main__":
    ...
    env = RefuelingEnv()
    n_states = env.state_space_shape
    ...
    for i in range(num_epochs):
        ...
        while not done and step < 50:
            ...
            action = env.get_index_by_action(action)
            # 将数据添加到经验池
        ...
        # 保存每个回合return
        episode_reward.append(epochs_return)

        # 打印回合信息
        print(f'iter:{i}, return:{np.mean(episode_reward[-10:])}')

    episode_reward = np.array(episode_reward)
    np.save('episode_reward_sac.npy', episode_reward)